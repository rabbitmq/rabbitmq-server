...
<<<<<<< HEAD
=======
 * The HTTP API now returns 404 Not Found instead of 401 Unauthorized when a user is not authorized to access a virtual host,
   to avoid disclosing virtual host existence.

   GitHub issue: [#16709](https://github.com/rabbitmq/rabbitmq-server/pull/16709)

 * Token expiry is now accepted as a float as well as an integer

   GitHub issue: [#16709](https://github.com/rabbitmq/rabbitmq-server/pull/16709)

 * List of all user limits now can only be accessed by the users tagged as `administrator`

   GitHub issue: [#16709](https://github.com/rabbitmq/rabbitmq-server/pull/16709)

 * Individual user limits page now can only be accessed by the users tagged as `administrator` or `monitoring`

   GitHub issue: [#16709](https://github.com/rabbitmq/rabbitmq-server/pull/16709)

>>>>>>> b040713 (Update `4.3.4` release notes)
#### Enhancements
