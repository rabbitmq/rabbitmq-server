...
const clientId = process.env.PROMETHEUS_MONITORING_CLIENT_ID
const clientSecret = process.env.PROMETHEUS_MONITORING_CLIENT_SECRET

describe('Prometheus endpoint with a monitoring-tagged OAuth2 client', function () {
  let token
  ...
  before(function () {
    ...
    assert.ok(clientId, 'PROMETHEUS_MONITORING_CLIENT_ID env var must be set')
    assert.ok(clientSecret, 'PROMETHEUS_MONITORING_CLIENT_SECRET env var must be set')
  ...
  it('can scrape /metrics', function () {
