...
describe('Overview page for a monitoring user', function () {
  ...
  it('shows the full Overview, including monitor-only sections', async function () {
    ...
    assert.ok(await overview.isGlobalCountDisplayed('Consumers'))

    assert.ok(await overview.isSectionDisplayed('Nodes'))
    assert.ok(await overview.isSectionDisplayed('Churn statistics'))
    assert.ok(await overview.isSectionDisplayed('Ports and contexts'))
