...
describe('Publish message section', function () {
  ...
  QUEUE_TYPES.forEach((type) => {
    describe(`given a ${type} queue`, function () {
      ...
      before(async function () {
        queueName = 'test_publish_' + type + '_' + Math.floor(Math.random() * 1000)
      ...
      it('should be displayed', async function () {
        assert.ok(await queuePage.isSectionDisplayed('Publish message'))
      })
