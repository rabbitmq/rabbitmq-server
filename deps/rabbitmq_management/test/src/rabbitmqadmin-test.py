...
""", ['--format', 'bash', 'list', 'vhosts', 'name', 'tracing'])
    ...
    def test_publish(self):
        ...
        self.assert_table([exp_msg('test', 2, False, 'test_1')], ['get', 'queue=test', 'ackmode=ack_requeue_false'])
        self.assert_table([exp_msg('test', 1, False, 'test_2')], ['get', 'queue=test', 'ackmode=ack_requeue_true'])
        self.assert_table([exp_msg('test', 1, True,  'test_2')], ['get', 'queue=test', 'ackmode=ack_requeue_false'])
        self.assert_table([exp_msg('test', 0, False, 'test_3')], ['get', 'queue=test', 'ackmode=ack_requeue_false'])
        self.run_success(['publish', 'routing_key=test'], stdin=b'test_4')
        filename = os.path.join(os.getenv("NODE_TMPDIR"), 'get.txt')
        ensure_dir(filename)
        self.run_success(['get', 'queue=test', 'ackmode=ack_requeue_false', 'payload_file=' + filename])
