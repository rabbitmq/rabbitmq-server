...
-export([augment_exchanges/3, augment_queues/3,
         ...
         get_all_connections/0, get_all_channels/0, get_connection/1, get_channel/1]).
...
get_channel(Name) ->
    submit(fun(_Interval) ->
                   created_stats_delegated(Name, channel_created_stats)
           end).
...
get_all_channels() ->
    submit(fun(_) ->
                   created_stats_delegated(channel_created_stats)
           end).
...
init([]) ->
    ...
    ?LOG_INFO("Statistics database started."),
...
merge_data(_, D1, D2) -> % we assume if we get here both values a maps
   try
       ...
       error:Err ->
           ?LOG_DEBUG("merge_data err ~tp got: ~tp ~tp", [Err, D1, D2]),
           case is_map(D1) of
               true -> D1;
               false -> D2
           end
...
delegate_invoke(FunOrMFA) ->
    ...
    case Errors of
        [] -> ok;
        _ -> ?LOG_WARNING("Management delegate query returned errors:~n~tp", [Errors])
    end,
    [R || {_, R} <- Results].
