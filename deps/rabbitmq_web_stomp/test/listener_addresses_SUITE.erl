...
-module(listener_addresses_SUITE).
...
no_address_is_reported_after_the_plugin_stops(Config) ->
    ...
    ok = rabbit_ct_broker_helpers:disable_plugin(Config, 0, "rabbitmq_web_stomp"),
    ?assertEqual([], reported_addresses(Config, Port)),
    ok = rabbit_ct_broker_helpers:enable_plugin(Config, 0, "rabbitmq_web_stomp"),
    ?assertEqual(configured_addresses(Config, Port),
                 reported_addresses(Config, Port)).
