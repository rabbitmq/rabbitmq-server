...
Subscribe => Key Version CorrelationId SubscriptionId Stream OffsetSpecification Credit Properties
  ...
  Credit => uint16 // credit in chunks
  Properties => [Property]
  Property => Key Value
  Key => string
  Value => string
...
1. Use version 1 for credit in chunks, version 2 for credit in bytes. The unit is
   fixed for the lifetime of the subscription. The response stays at version 1
   regardless of the request version.
...
A client subscribed with credit in bytes (`Subscribe` version 2) can compute the
cost of a delivered chunk from the `Deliver` frame size alone, with no extra
field on the wire: `FrameSize - 5` for version 1, `FrameSize - 13` for version 2.
...
Credit => Key Version SubscriptionId Credit
  ...
  Version => uint16
  SubscriptionId => uint8
  Credit => uint32 // the number of bytes that can be sent
...
1. Use version 1 for credit in chunks, version 2 for credit in bytes. This must
   match the unit the subscription was created with: a `Credit` frame whose
   version disagrees with the subscription is answered with a `CreditResponse`
   carrying response code "Precondition failed", and the credit it carried is
   dropped, not applied. The response stays at version 1 regardless of the
   request version.

NB: the server sent a response only in case of problem, e.g. crediting an unknown
subscription or a unit mismatch.

For credit in bytes, the server may start sending a chunk as soon as credit is
positive, and only stops once credit is not positive, rather than waiting for
enough credit to cover the whole chunk. This overshoot rule means a chunk larger
than the client's credit window is still delivered instead of stalling the
subscription, and the amount of data outstanding at any point is bounded by the
window size plus the largest chunk in the stream. A client subscribing with
credit in bytes must be prepared to receive a chunk larger than the window it
granted, and must eventually grant credit for every byte it receives.
