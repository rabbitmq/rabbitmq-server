...
%% returns frames
-spec incoming_data(binary(), state()) -> {state(), [command()]}.
...
incoming_data(Data,
    ...
    {State#?MODULE{frames = [], data = Data}, parse_frames(Frames)};
incoming_data(Data,
              #?MODULE{frames = Frames, data = {Size, Partial}} = State) ->
    case Data of
...
parse_frames(Frames) ->
    lists:foldl(fun(Frame, Acc) -> [parse_command(Frame) | Acc] end, [],
                Frames).
