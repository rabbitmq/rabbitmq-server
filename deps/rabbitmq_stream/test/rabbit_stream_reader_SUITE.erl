...
clean_state_after_partial_super_stream_deletion_test(_) ->
    Mod = rabbit_stream_reader,
    DeletedPartition = <<"deleted-partition">>,
    FailedPartition = <<"failed-partition">>,
    ModTransport = dummy_transport,
    meck:new(ModTransport, [non_strict]),
    meck:expect(ModTransport, send, 2, ok),
    ModMetrics = rabbit_stream_metrics,
    meck:new(ModMetrics, [non_strict]),
    meck:expect(ModMetrics, consumer_cancelled, 4, ok),
    meck:expect(ModMetrics, publisher_deleted, 3, ok),
    ModCounters = rabbit_global_counters,
    meck:new(ModCounters, [non_strict]),
    meck:expect(ModCounters, increase_protocol_counter, 3, ok),
    ModLog = osiris_log,
    meck:new(ModLog, [non_strict]),
    meck:expect(ModLog, init, 1, ok),
    put(close_log_count, 0),
    meck:expect(ModLog, close,
                fun(_) -> put(close_log_count, get(close_log_count) + 1) end),
    ModCore = rabbit_stream_core,
    meck:new(ModCore, [non_strict]),
    put(metadata_update, []),
    meck:expect(ModCore, frame,
                fun(Cmd) -> put(metadata_update, [Cmd | get(metadata_update)]) end),
    C0 = #stream_connection{
           publishers = #{0 => #publisher{stream = DeletedPartition},
                          1 => #publisher{stream = FailedPartition}},
           stream_subscriptions = #{DeletedPartition => [0],
                                    FailedPartition => [1]},
           user = #user{}},
    S0 = #stream_connection_state{
           consumers = #{0 => consumer(DeletedPartition),
                         1 => consumer(FailedPartition)}},
    {C1, S1} = Mod:clean_state_after_super_stream_deletion(
                 [DeletedPartition], C0, S0, ModTransport, ignored_socket),
    ?assertEqual(1, get(close_log_count)),
    erase(close_log_count),
    ?assertEqual([{metadata_update, DeletedPartition,
                   ?RESPONSE_CODE_STREAM_NOT_AVAILABLE}],
                 get(metadata_update)),
    erase(metadata_update),
    ?assertEqual(#{1 => #publisher{stream = FailedPartition}},
                 C1#stream_connection.publishers),
    ?assertEqual(#{FailedPartition => [1]},
                 C1#stream_connection.stream_subscriptions),
    ?assertEqual(#{1 => consumer(FailedPartition)},
                 S1#stream_connection_state.consumers),
    ok.

delete_super_stream_partial_failure_protocol_response_test(_) ->
    Mod = rabbit_stream_reader,
    SuperStream = <<"super-stream">>,
    DeletedPartition = <<"deleted-partition">>,
    FailedPartition = <<"failed-partition">>,
    ModTransport = dummy_transport,
    meck:new(ModTransport, [non_strict]),
    meck:expect(ModTransport, send, 2, ok),
    ModUtils = rabbit_stream_utils,
    meck:new(ModUtils, [non_strict]),
    meck:expect(ModUtils, check_super_stream_permitted, 3, ok),
    meck:expect(ModUtils, check_super_stream_management_permitted, 4, ok),
    ModManager = rabbit_stream_manager,
    meck:new(ModManager, [non_strict]),
    meck:expect(ModManager, partitions, 2,
                {ok, [DeletedPartition, FailedPartition]}),
    meck:expect(ModManager, delete_super_stream, 4,
                {error, {partitions_not_deleted,
                         [DeletedPartition],
                         [{FailedPartition, timeout}]}}),
    ModMetrics = rabbit_stream_metrics,
    meck:new(ModMetrics, [non_strict]),
    meck:expect(ModMetrics, consumer_cancelled, 4, ok),
    meck:expect(ModMetrics, publisher_deleted, 3, ok),
    ...
    meck:expect(ModCore, frame,
                fun(Cmd) ->
                        put(protocol_frames, [Cmd | get(protocol_frames)]),
                        Cmd
                end),
    Connection0 =
        #stream_connection{socket = ignored_socket,
                           virtual_host = <<"/">>,
    ...
    State0 =
        #stream_connection_state{consumers =
                                     #{0 => consumer(DeletedPartition),
                                       1 => consumer(FailedPartition)}},
    {Connection1, State1} =
        Mod:handle_frame_post_auth(
          ModTransport, Connection0, State0,
          {request, 42, {delete_super_stream, SuperStream}}),
    ?assert(lists:member({response, 42,
                          {delete_super_stream, ?RESPONSE_CODE_INTERNAL_ERROR}},
                         get(protocol_frames))),
    ?assert(lists:member({metadata_update, DeletedPartition,
                          ?RESPONSE_CODE_STREAM_NOT_AVAILABLE},
                         get(protocol_frames))),
    erase(protocol_frames),
    ?assertEqual(1, get(close_log_count)),
    erase(close_log_count),
    ?assertEqual(#{1 => #publisher{stream = FailedPartition}},
    ...
    ?assertEqual(#{1 => consumer(FailedPartition)},
                 State1#stream_connection_state.consumers),
    ok.

delete_super_stream_exchange_failure_protocol_response_test(_) ->
    Mod = rabbit_stream_reader,
    SuperStream = <<"super-stream">>,
    ...
    %% All partitions were deleted, but the super stream exchange was not,
    %% so the manager reports every partition as already gone.
    meck:expect(ModManager, delete_super_stream, 4,
                {error, {exchange_not_deleted,
    ...
    Connection0 =
        #stream_connection{socket = ignored_socket,
                           virtual_host = <<"/">>,
                           user = #user{username = <<"guest">>},
                           publishers = #{0 => #publisher{stream = Partition1},
                                          1 => #publisher{stream = Partition2}},
                           stream_subscriptions = #{Partition1 => [0],
                                                    Partition2 => [1]}},
    State0 =
        #stream_connection_state{consumers =
                                     #{0 => consumer(Partition1),
                                       1 => consumer(Partition2)}},
    {Connection1, State1} =
        Mod:handle_frame_post_auth(
          ModTransport, Connection0, State0,
          {request, 42, {delete_super_stream, SuperStream}}),
    ?assert(lists:member({response, 42,
                          {delete_super_stream, ?RESPONSE_CODE_INTERNAL_ERROR}},
                         get(protocol_frames))),
    ?assert(lists:member({metadata_update, Partition1,
                          ?RESPONSE_CODE_STREAM_NOT_AVAILABLE},
                         get(protocol_frames))),
    ?assert(lists:member({metadata_update, Partition2,
                          ?RESPONSE_CODE_STREAM_NOT_AVAILABLE},
                         get(protocol_frames))),
    erase(protocol_frames),
    %% Both partitions were already deleted, so their state is cleaned up
    %% even though the overall operation is reported as a failure.
    ?assertEqual(2, get(close_log_count)),
    erase(close_log_count),
    ?assertEqual(#{}, Connection1#stream_connection.publishers),
    ?assertEqual(#{}, Connection1#stream_connection.stream_subscriptions),
    ?assertEqual(#{}, State1#stream_connection_state.consumers),
    ok.
