...
roundtrip(_Config) ->
    ...
    test_roundtrip({publish_error, 42,
                    ?RESPONSE_CODE_STREAM_DOES_NOT_EXIST, [2,3,4]}),
    test_roundtrip({deliver, 53, <<"chunk">>}),
    test_roundtrip({credit, 53, 12}),
    test_roundtrip({metadata_update, <<"stream1">>, ?RESPONSE_VHOST_ACCESS_FAILURE}),
    test_roundtrip(heartbeat),
    test_roundtrip({tune, 53, 12}),
    %% REQUESTS
    test_roundtrip({request, 99,
                    {declare_publisher, 42, <<"writer_ref">>, <<"stream_name">>}}),
    test_roundtrip({request, 99,
                    {query_publisher_sequence, <<"writer_ref">>, <<"stream_name">>}}),
    test_roundtrip({request, 99, {delete_publisher, 42}}),
    [test_roundtrip({request, 99, {subscribe, 53, <<"stream_name">>, Spec, 12}})
     || Spec <- [last, next, first, 65432, {timestamp, erlang:system_time(millisecond)}]],
    test_roundtrip({request, 99, {commit_offset, <<"offset_ref">>,  <<"stream">>, 12}}),
    test_roundtrip({request, 99, {query_offset, <<"offset_ref">>,  <<"stream">>}}),
    test_roundtrip({request, 99, {unsubscribe, 53}}),
    Args = #{<<"arg1">> => <<"arg1_value">>},
    test_roundtrip({request, 99, {create_stream, <<"stream_name">>, Args}}),
    test_roundtrip({request, 99, {create_stream, <<"stream_name">>, #{}}}),
    test_roundtrip({request, 99, {delete_stream, <<"stream_name">>}}),
    test_roundtrip({request, 99, {metadata, [<<"stream1">>, <<"stream2">>]}}),
    test_roundtrip({request, 99, {peer_properties, #{<<"k1">> => <<"v1">>}}}),
    test_roundtrip({request, 99, sasl_handshake}),
    test_roundtrip({request, 99, {sasl_authenticate, <<"mechanism">>, <<>>}}),
    test_roundtrip({request, 99, {sasl_authenticate, <<"mechanism">>, <<"challenge">>}}),
    ...
    [test_roundtrip({response, 99, {Tag, 53}})
     || Tag <- [declare_publisher, delete_publisher, subscribe, unsubscribe,
                create_stream, delete_stream, open, close]],

    test_roundtrip({response, 99, {query_publisher_sequence, 98, 1234}}),
    test_roundtrip({response, 99, {query_offset, 1, 12}}),

    test_roundtrip({response, 99, {peer_properties, 1, #{<<"k1">> => <<"v1">>}}}),

    test_roundtrip({response, 99, {sasl_handshake, 1, [<<"m1">>, <<"m2">>]}}),
    test_roundtrip({response, 99, {sasl_authenticate, 1, <<"challenge">>}}),
...
roundtrip_metadata(_Config) ->
    ...
    Metadata = #{<<"stream1">> => {Host1, [Host2, Host3]},
...
roundtrip_metadata_no_leader(_Config) ->
    ...
    Host3 =  {<<"host3">>, 1236},
    Endpoints = [Host1, Host2, Host3],
    Metadata = #{<<"stream1">> => {undefined, [Host2, Host3]},
                 <<"stream2">> => stream_not_found,
                 <<"stream3">> => stream_not_available},
