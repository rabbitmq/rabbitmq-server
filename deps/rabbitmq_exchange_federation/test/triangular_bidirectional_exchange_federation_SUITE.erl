...
setup_vhosts(Config) ->
    %% Be defensive: delete vhosts first in case they exist from a previous failed runs
...
setup_federation_triangle(Config) ->
    ...
    %% Create upstreams: three virtual hosts, each will bidirectionally federated with
    %% the other two.
    lists:foreach(
      fun(LocalVHost) ->
              lists:foreach(
    ...
    %% Each virtual hosts needs its own policy that enables federation
    lists:foreach(
      fun(VHost) ->
    ...
    %% Wait for all 6 federation links to be up and running
...
triangle_federation_with_messages(Config) ->
    %% Verify federation links are running
    ?assertEqual(6, count_running_links(Config)),

    %% Publish messages randomly across vhosts using publisher confirms
    MessagesByVHost = publish_messages_with_confirms(Config, ?MESSAGE_COUNT),
    ct:pal("Published messages by vhost: ~p", [MessagesByVHost]),

    %% Wait for messages to be federated to all queues.
    %% Each message creates 15 copies (explained below), so wait for 500 * 15 = 7500 messages.
...
publish_messages_with_confirms(Config, Count) ->
    %% Open a channel per vhost with confirms enabled
    Channels = lists:map(
                 fun(VHost) ->
                         {ok, Ch} = open_channel_to_vhost(Config, VHost),
...
publish_messages_with_confirms_multi_node(Config, Count) ->
    Nodes = [0, 1, 2],
    %% Open a channel per node with confirms enabled
    Channels = lists:map(
                 fun(Node) ->
                         {ok, Ch} = open_channel_to_node(Config, Node),
                         #'confirm.select_ok'{} = amqp_channel:call(Ch, #'confirm.select'{}),
