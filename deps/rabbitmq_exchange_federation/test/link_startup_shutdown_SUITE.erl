...
%% Verifies that exchange federation links start and stop in response to
...
-define(BINDING_QUEUE, <<"fed.x.binding-queue">>).
-define(POLICY_NAME, <<"federate-exchanges">>).
...
link_stops_after_policy_disables_federation(Config) ->
    declare_exchanges_and_binding(Config),
...
declare_exchanges_and_binding(Config) ->
    ...
    %% Declare the exchange in the upstream vhost
    {UpConn, UpCh} = open_channel_to_vhost(Config, UpVHost),
    try
        #'exchange.declare_ok'{} =
            amqp_channel:call(UpCh, #'exchange.declare'{
                                       exchange = ?EXCHANGE_NAME,
                                       type = <<"topic">>,
                                       durable = true})
    after
        close_connection_and_channel(UpConn, UpCh)
    end,

    %% Declare the exchange, a queue, and a binding in the downstream vhost.
    %% Exchange federation links only start when there are downstream bindings.
    {DownConn, DownCh} = open_channel_to_vhost(Config, DownVHost),
    try
        #'exchange.declare_ok'{} =
            amqp_channel:call(DownCh, #'exchange.declare'{
                                         exchange = ?EXCHANGE_NAME,
                                         type = <<"topic">>,
                                         durable = true}),
        #'queue.declare_ok'{} =
            amqp_channel:call(DownCh, #'queue.declare'{
                                         queue = ?BINDING_QUEUE,
                                         durable = true,
                                         arguments = [{<<"x-queue-type">>, longstr, <<"classic">>}]}),
        #'queue.bind_ok'{} =
            amqp_channel:call(DownCh, #'queue.bind'{
                                         queue = ?BINDING_QUEUE,
                                         exchange = ?EXCHANGE_NAME,
                                         routing_key = <<"#">>})
    after
        close_connection_and_channel(DownConn, DownCh)
    end.
