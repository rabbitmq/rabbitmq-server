...
%% Forward a link's blocked_buffer, stopping when it is empty or when the link
%% becomes blocked again, either by the downstream connection raising a resource
%% alarm or by credit_flow entering flow state after a delivery. Returns the
%% remaining queue and the updated state; the caller stashes the queue back into
