...
-module(web_mqtt_login_timeout_SUITE).
-compile([export_all]).
...
init_per_testcase(_Testcase, Config) ->
    rabbit_ct_broker_helpers:rpc(
      Config, 0,
      application, set_env, [rabbitmq_mqtt, login_timeout, 400]),
    Config.

end_per_testcase(_Testcase, Config) ->
    rabbit_ct_broker_helpers:rpc(
      Config, 0,
      application, unset_env, [rabbitmq_mqtt, login_timeout]),
    Config.

%% A WebSocket client that never sends an MQTT CONNECT packet must be
...
%% A client that sends CONNECT before the timeout must not be
...
authenticated_connection_survives_timeout(Config) ->
    ...
    timer:sleep(600),
    %% Connection must still be alive. Send DISCONNECT and expect clean close.
