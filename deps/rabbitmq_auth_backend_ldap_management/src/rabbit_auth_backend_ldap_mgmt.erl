...
accept_content(ReqData0, Context) ->
    F = fun (_Values, BodyMap, ReqData1) ->
        try
            ...
            UserDN = rabbit_data_coercion:to_unicode_charlist(maps:get(user_dn, BodyMap, <<"">>)),
            Password = rabbit_data_coercion:to_unicode_charlist(maps:get(password, BodyMap, <<"">>)),
            Options0 = [
                {port, Port},
                {timeout, 5000}
            ],
...
to_unicode(undefined) ->
    undefined;
to_unicode(Arg) when is_atom(Arg) ->
    Arg;
to_unicode(Arg) ->
