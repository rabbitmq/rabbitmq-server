...
-record(registry_state, {selectors}).
...
init([]) ->
    {ok, #registry_state{selectors = []}}.

handle_call({add, Selector, Handler}, _From, 
            State = #registry_state{selectors = Selectors}) ->
    UpdatedState =  State#registry_state{selectors = Selectors ++ [{Selector, Handler}]},
