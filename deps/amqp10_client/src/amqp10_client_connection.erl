...
send_open(#state{socket = Socket, config = Config0}) ->
    ...
    Open = case Config of
               #{desired_capabilities := DesCapsList} when is_list(DesCapsList) ->
                   DesCaps = amqp10_util:capabilities_from_list(DesCapsList),
                   Open1#'v1_0.open'{desired_capabilities = DesCaps};
               _ ->
                   Open1
           end,
