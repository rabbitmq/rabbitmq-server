...
run_not_federated(Config) ->
    ...
    Opts = #{node => A},
    {stream, []} = ?CMD:run([], Opts#{down => false}).
...
run_federated(Config) ->
    ...
    rabbit_federation_test_util:with_ch(
      Config,
      fun(_) ->
              {stream, [Props]} = ?CMD:run([], Opts#{down => false}),
...
run_down_federated(Config) ->
    ...
    rabbit_federation_test_util:with_ch(
      Config,
      fun(_) ->
              {stream, ManyProps} = ?CMD:run([], Opts#{down => false}),
    ...
    rabbit_federation_test_util:with_ch(
      Config,
      fun(_) ->
              {stream, [Props]} = ?CMD:run([], Opts#{down => true}),
