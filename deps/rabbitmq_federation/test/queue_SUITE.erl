...
-import(rabbit_federation_test_util,
        [expect/3, expect/4,
         ...
         await_running_queue_federation/3]).
...
simple(Config) ->
    with_ch(Config,
      fun (Ch) ->
              await_running_queue_federation(Config,
...
message_flow(Config) ->
    ...
    with_ch(Config,
      fun (Ch) ->
              ...
              expect_empty(Ch, <<"one">>),
              expect_empty(Ch, <<"two">>),
              %% Clear federation configuration to avoid a race condition when
              %% deleting the queues in quorum mode
...
supervisor_shutdown_concurrency_safety(Config) ->
    ...
    with_ch(Config,
      fun (Ch) ->
          ...
          %% Test stop_child/1 by deleting a federated queue while the supervisor
          %% is down
...
<<<<<<< HEAD:deps/rabbitmq_federation/test/queue_SUITE.erl
...
>>>>>>> 819c87c44 (Attempt to deflake queue federation tests):deps/rabbitmq_queue_federation/test/queue_SUITE.erl
          expect_federation(Ch, UpQ, DownQ, ?EXPECT_FEDERATION_TIMEOUT)
      end, upstream_downstream(Config)).
