...
%   #'access.request_ok'{ticket = Ticket} = amqp_channel:rpc(Channel, AccessRequest)
%   %%...do something useful
%   ChannelClose = #'channel.close'{ %% set the appropriate fields },
%   amqp_channel:rpc(Channel, ChannelClose),
...
lifecycle_test(Connection) ->
    ...
    #'queue.declare_ok'{queue = Q1,
                       ...
                       = amqp_channel:rpc(Channel,QueueDeclare),
    ?assertMatch(Q, Q1),
    ExchangeDeclare = #'exchange.declare'{ticket = Ticket, exchange = X, type = <<"topic">>,
                                          ...
                                          nowait = false, arguments = []},
    #'exchange.declare_ok'{} = amqp_channel:rpc(Channel, ExchangeDeclare),
    QueueBind = #'queue.bind'{ticket = Ticket, queue = Q, exchange = X,
                              routing_key = BindKey, nowait = false, arguments = []},
    #'queue.bind_ok'{} = amqp_channel:rpc(Channel, QueueBind),
    QueueDelete = #'queue.delete'{ticket = Ticket, queue = Q,
                                  if_unused = true, if_empty = true, nowait = false},
    #'queue.delete_ok'{message_count = MessageCount2} = amqp_channel:rpc(Channel, QueueDelete),
    ...
    #'exchange.delete_ok'{} = amqp_channel:rpc(Channel, ExchangeDelete),
...
basic_get_test(Connection) ->
    ...
    {Method, Content} = amqp_channel:rpc(Channel, BasicGet),
    ...
    {Method2, Content2} = amqp_channel:rpc(Channel, BasicGet),
...
basic_ack_test(Connection) ->
    ...
    ok = amqp_channel:send(Channel, BasicAck),
    teardown(Connection, Channel).
...
basic_consume_test(Connection) ->
    ...
    #'basic.consume_ok'{consumer_tag = ConsumerTag} = amqp_channel:rpc(Channel,BasicConsume, Consumer),
    receive
    after 2000 ->
        BasicCancel = #'basic.cancel'{consumer_tag = ConsumerTag, nowait = false},
        #'basic.cancel_ok'{consumer_tag = ConsumerTag} = amqp_channel:rpc(Channel,BasicCancel),
...
setup_publish(Connection) ->
    ...
    amqp_channel:send(Channel, BasicPublish, Content),
    {Channel,Ticket,Q}.
...
teardown({ConnectionPid, Mode}, Channel) ->
    ...
    #'channel.close_ok'{} = amqp_channel:rpc(Channel, ChannelClose),
...
setup_channel(Connection, Realm) ->
    ...
    Access = #'access.request'{realm = Realm,
                               ...
                               write = true,
                               read = true},
    #'access.request_ok'{ticket = Ticket} = amqp_channel:rpc(Channel, Access),
    {Channel, Ticket}.
