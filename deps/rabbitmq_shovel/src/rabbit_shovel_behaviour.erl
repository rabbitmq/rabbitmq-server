...
decr_remaining(N, State = #{source := #{remaining := M} = Src,
    ...
    case M > N of
        ...
        false ->
            _ = rabbit_log_shovel:info("shutting down Shovel '~s', no messages left to transfer~n", [Name]),
            _ = rabbit_log_shovel:debug("shutting down Shovel '~s', no messages left to transfer. Shovel state: ~p~n", [Name, State]),
