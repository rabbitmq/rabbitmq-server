...
stream_offset_header(_) ->
    ...
    lists:foreach(fun({Header, Expected}) ->
        ?assertEqual(
            Expected,
            rabbit_stomp_frame:stream_offset_header(#stomp_frame{headers = maps:from_list([Header])})
            )
    end, TestCases).
...
stream_filter_header(_) ->
    ...
    lists:foreach(fun({Header, Expected}) ->
        ?assertEqual(
            Expected,
            rabbit_stomp_frame:stream_filter_header(#stomp_frame{headers = maps:from_list([Header])})
        )
    end, TestCases).
