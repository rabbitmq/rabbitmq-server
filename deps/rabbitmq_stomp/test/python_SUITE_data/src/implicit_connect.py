...
class TestImplicitConnect(base.BaseTest):
    ...
    def test_no_implicit_connec_erros(self):
        ''' When implicit ocnnect turned off and first frame is not CONNECT, server returns ERROR and closes'''

        self.conn.disconnect()
        # test_util.disable_implicit_connect()  assuming it desabled by default
        listener = base.WaitableListener()
        new_conn = stomp.Connection(host_and_ports=[('localhost', int(os.environ["STOMP_PORT"]))])
        new_conn.set_listener('', listener)
        ...
        try:
            self.assertTrue(listener.wait(5))
            self.assertEqual(1, len(listener.errors),
                             "Didn't get Error, why?")
            self.assertEqual('Illegal command "SUBSCRIBE"', listener.errors[0]['headers']['message'])
            time.sleep(1)
