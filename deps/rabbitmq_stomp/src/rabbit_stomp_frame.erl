...
parse_unknown_body(Content, Frame, Chunks, Remaining) ->
    case firstnull(Content) of
        -1  ->
            ...
            ?MORE_BODY(Content, Frame, Chunks, Remaining - ChunkSize);
        Pos ->
...
%% parse_body1(Content, Frame, Chunks, MaxRemaining, Remaining) ->
%%     Size = byte_size(Content),
%%     {MoreOrDone, Left, ChunkSize} = case Remaining of
%%                                       unknown ->
%%                                           case firstnull(Content) of
%%                                               -1  -> {more, unknown, Size};
%%                                               Pos -> {done, Pos, Pos}
...
%%                                       _ -> 
%%                                           case Remaining >= Size of
%%                                               true  -> {more, Remaining - Size, Size};
%%                                               false -> {done, Remaining, Remaining}
%%                                           end
%%                                   end,

%%     case ChunkSize > MaxRemaining of
%%         true ->  error({error, {max_body_length, unknown}});
%%         false -> ok
%%     end,
...
%%         more ->
%%             Chunks1 = finalize_chunk(Content, Chunks),
%%             more(fun(Rest) -> parse_body1(Rest, Frame, Chunks1, MaxRemaining - ChunkSize, {more, Left}) end);
%%         done -> 
%%             <<Chunk:Left/binary, 0, Rest/binary>> = Content,
%%             Body = lists:reverse(finalize_chunk(Chunk, Chunks)),
%%             {ok, Frame#stomp_frame{body_iolist = Body}, Rest}
%%     end.
