...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, start_link/1]).
...
-record(?STATE,
        {store_state :: rabbit_mqtt_retained_msg_store:state(),
         expire :: #{topic() := TimerRef :: reference()}}).

-type state() :: #?STATE{}.

-spec start_link(rabbit_types:vhost()) -> gen_server:start_ret().
start_link(VHost) ->
...
-spec fetch(pid(), topic()) -> undefined | mqtt_msg() | [mqtt_msg()].
fetch(Pid, Topic) ->
...
-spec init(rabbit_types:vhost()) -> {ok, state()}.
init(VHost) ->
    process_flag(trap_exit, true),
    {StoreState, Expire0} = rabbit_mqtt_retained_msg_store:start(VHost),
    Now = os:system_time(second),
    Expire =
        maps:map(fun(Topic, {Timestamp, Expiry}) ->
                    TimerSecs = max(0, Expiry - (Now - Timestamp)),
                    start_timer(TimerSecs, Topic)
                 end,
                 Expire0),
    {ok, #?STATE{store_state = StoreState, expire = Expire}}.

handle_cast({retain, Topic, Msg0 = #mqtt_msg{props = Props}},
            State = #?STATE{store_state = StoreState, expire = Expire0}) ->
    Expire2 =
        case maps:take(Topic, Expire0) of
            {OldTimer, Expire1} ->
    ...
    {Msg, Expire} =
        case maps:find('Message-Expiry-Interval', Props) of
            {ok, ExpirySeconds} ->
...
handle_call({fetch, Topic}, _From, State = #?STATE{store_state = StoreState}) ->
    Reply =
        case rabbit_mqtt_retained_msg_store:lookup(Topic, StoreState) of
            [] ->
                undefined;
            Messages when is_list(Messages) ->
                lists:map(fun update_message_expiry/1, Messages);
            SingleMessage = #mqtt_msg{} ->
                update_message_expiry(SingleMessage);
            Other ->
                Other
        end,
    {reply, Reply, State}.

-spec update_message_expiry(mqtt_msg()) -> mqtt_msg().
update_message_expiry(#mqtt_msg{props = #{'Message-Expiry-Interval' := Expiry0} = Props,
                                timestamp = Timestamp} =
                          MqttMsg) ->
...
update_message_expiry(MqttMsg) ->
    MqttMsg.

handle_info({timeout, Timer, Topic},
            State = #?STATE{store_state = StoreState, expire = Expire0}) ->
    Expire =
        case maps:take(Topic, Expire0) of
            {Timer, Expire1} ->
...
start_timer(Seconds, Topic) when is_binary(Topic) ->
    erlang:start_timer(
        timer:seconds(Seconds), self(), Topic).
