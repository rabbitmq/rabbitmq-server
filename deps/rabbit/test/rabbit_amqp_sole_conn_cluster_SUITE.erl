...
status_should_not_crash_when_never_bootstrapped(Config) ->
    ...
    %% Simulate Node1 believing it is the only reachable node, e.g. a
    %% single-node deployment where the store has never been started.
    call(Config, Node1, meck, expect,
         [rabbit_nodes, list_reachable, fun() -> [Node1] end]),
...
concurrent_acquire_should_not_crash_on_lazy_bootstrap(Config) ->
    ...
    NumConns = 30,
    Results = call(Config, Node1, ?MODULE, run_concurrent_acquires, [NumConns]),
    ct:pal("Concurrent acquire results: ~p", [Results]),

    ?assertEqual(NumConns, length(Results)),
    ...
    call(Config, Node, meck, expect,
         [rabbit_sup, start_child,
          fun(?SOLE_CONN_MOD) ->
                  ...
                  %% concurrent caller won the race to register the name.
                  case gen_server:start({local, ?SOLE_CONN_MOD},
                                        ?SOLE_CONN_MOD, [], []) of
                      {ok, _Pid} -> ok;
                      {error, _} = Err -> Err
                  end
...
%% Runs entirely on the target node (invoked via peer:call/erpc:call).
%% Spawns Count workers, releases them all at once with a barrier so their
%% acquire/5 calls race against each other while the local store is still
%% lazily bootstrapping, and returns each worker's result.
run_concurrent_acquires(Count) ->
    Parent = self(),
    Workers =
        [begin
             ConnPid = erlang:spawn(fun() -> receive die -> ok end end),
             CID = iolist_to_binary(io_lib:format("concurrent-~b", [I])),
             WorkerPid = erlang:spawn(
                           fun() ->
                                   receive go -> ok end,
                                   Result =
                                       try
                                           ?SOLE_CONN_MOD:acquire(
                                              refuse_connection, ?VH, CID, ?USER, ConnPid)
                                       catch
                                           Class:Reason ->
    ...
    [WorkerPid ! go || {WorkerPid, _ConnPid} <- Workers],
    Results = [receive {WorkerPid, R} -> R end || {WorkerPid, _ConnPid} <- Workers],
    [erlang:exit(ConnPid, kill) || {_WorkerPid, ConnPid} <- Workers],
    Results.
