...
all_tests() ->
    [
     ...
     refuse_connection_conflict_should_refuse_new_connection_default,
     refuse_connection_conflict_should_refuse_new_connection_explicit,
     ...
     sole_conn_gen_server_supervised
    ].
...
refuse_connection_conflict_should_refuse_new_connection_default(Config) ->
    %% refuse_connection is the default policy
    ContainerId = atom_to_binary(?FUNCTION_NAME),
    BaseOpnConf = #{container_id => ContainerId},
    refuse_connection_conflict_should_refuse_new_connection(Config, BaseOpnConf).

refuse_connection_conflict_should_refuse_new_connection_explicit(Config) ->
    ContainerId = atom_to_binary(?FUNCTION_NAME),
    BaseOpnConf = #{container_id => ContainerId,
                    properties => #{?SOLE_CONN_ENFORCEMENT_POLICY_KEY =>
                                    ?SOLE_CONN_ENFORCEMENT_POLICY_REFUSE_CONN}},
    refuse_connection_conflict_should_refuse_new_connection(Config, BaseOpnConf).

refuse_connection_conflict_should_refuse_new_connection(Config, BaseOpnConf) ->
    OpnConf0 = conn_config(first, Config),
    OpnConf1 = maps:merge(OpnConf0, BaseOpnConf),
    OpnConf2 = OpnConf1#{
                 desired_capabilities => [?CAP_SOLE_CONN],
                 notify_with_performative => true
                },
    {ok, Connection1} = amqp10_client:open_connection(OpnConf2),
    receive {amqp10_event, {connection, Connection1,
                            {opened, #'v1_0.open'{
                                        offered_capabilities = OffCaps1,
                                        properties = Props1}}}} ->
    ...
    OpnConfConn3 = maps:merge(OpnConf2, conn_config(other, Config)),
    {ok, Connection2} = amqp10_client:open_connection(OpnConfConn3),
    receive {amqp10_event, {connection, Connection2,
                            {opened, #'v1_0.open'{
                                        offered_capabilities = OffCaps2,
                                        properties = Props2}}}} ->
    ...
    receive {amqp10_event, {connection, Connection2,
                ...
                ?assertEqual({utf8,
                             ...
                             Desc),
                ?assert(lists:member({{symbol, <<"invalid-field">>},
                                      {symbol, <<"container-id">>}},
                                     Info))
...
sole_conn_gen_server_supervised(Config) ->
    NodeCount = proplists:get_value(rmq_nodes_count, Config, 1),
    Connections = [open_sole_conn_and_verify_supervised(Config, NodeIndex)
                   || NodeIndex <- lists:seq(0, NodeCount - 1)],
    lists:foreach(fun(Connection) -> close_connection_sync(Connection) end, Connections).

open_sole_conn_and_verify_supervised(Config, NodeIndex) ->
    ContainerId = <<(atom_to_binary(?FUNCTION_NAME))/binary,
                    "-", (integer_to_binary(NodeIndex))/binary>>,
    OpnConf0 = connection_config(NodeIndex, Config),
    OpnConf1 = OpnConf0#{
                 container_id => ContainerId,
                 desired_capabilities => [?CAP_SOLE_CONN],
                 notify_with_performative => true
    ...
    RabbitSupChildren = rabbit_ct_broker_helpers:rpc(
                          Config, NodeIndex, supervisor, which_children, [rabbit_sup]),
    ?assertMatch({rabbit_sole_conn_sup, _, supervisor, [rabbit_restartable_sup]},
                 lists:keyfind(rabbit_sole_conn_sup, 1, RabbitSupChildren)),
