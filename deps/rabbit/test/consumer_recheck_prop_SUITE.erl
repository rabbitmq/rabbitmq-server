...
%% Each consumer is randomly assigned to a queue and randomly marked
%% as allowed or denied.
consumer_setup_gen() ->
    ?LET(RawPairs,
         list({consumer_tag_gen(), queue_name_gen(), bool()}),
         begin
             %% Deduplicate by CTag (keep first occurrence)
...
%% This models the window between basic.consume (NoWait=false) and
%% the arrival of basic.consume_ok.
...
%%
%% This replicates the maps:fold in recheck_consumers/1 paired with
%% the state updates in cancel_consumer_recheck/3.  The fold iterates
%% over the *original* consumer_mapping snapshot while the accumulator
%% carries the progressively mutated state — exactly what the
...
%% Full bijection: consumer_mapping ↔ queue_consumers.
      ...
      fun() ->
              ?FORALL(Setup, consumer_setup_gen(),
                      begin
                          ...
                          %% Every denied consumer is gone
                          lists:all(fun(C) -> not maps:is_key(C, CMap1) end,
                                    DeniedCTags)
                          %% Every allowed consumer is retained
...
%% queue_consumers — the basic.consume NoWait=false window), the recheck
%% must not crash and the result must still satisfy the weak invariant.
      ...
      fun() ->
              ?FORALL(Setup, consumer_setup_with_pending_gen(),
                      begin
                          ...
                          {CMap1, QCons1} = apply_recheck(Denied, CMap, CMap, QCons),
                          %% Must not crash (gb_sets:delete_any handles missing elements)
                          %% and the weak invariant must hold.
                          is_weakly_consistent(CMap1, QCons1)
                          %% Denied consumers removed
                          andalso sets:fold(
