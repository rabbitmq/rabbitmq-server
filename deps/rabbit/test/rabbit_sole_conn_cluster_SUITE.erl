-module(rabbit_sole_conn_cluster_SUITE).
...
-define(SOLE_CONN_MOD, rabbit_sole_conn).
-define(SOLE_CONN_GEN_SERVER_SUP, rabbit_sole_conn_sup).
-define(STORE_ID, rabbit_sole_conn:get_store_id()).
-define(VH, <<"/">>).
-define(CID1, <<"id-1">>).
-define(CID2, <<"id-2">>).
-define(CID3, <<"id-3">>).
    ...
    lists:foreach(
      fun({_Node, Peer}) ->
              ok = peer:call(Peer, application,
                             set_env,
                             [rabbit, sole_connection_tick_interval, 1000])
      end, Nodes),
    ok;
...
node_rejoins_cluster_after_abrupt_shutdown(Config0) ->
    ...
    %% not-yet-evicted ghost identity (see rabbit_sole_conn:start_local_store/0);
