...
-export([all/0,
         ...
         replaced_delete_queue_transaction/1,
         replaced_delete_queue_transaction1/1
        ]).
...
delete_v2(Config) ->
    Ret = rabbit_ct_broker_helpers:enable_feature_flag(
            Config, replaced_delete_queue_transaction),
    case Ret of
        ok ->
...
delete1(_Config, Version) ->
    case Version of
        v1 ->
            ?assertNot(
               rabbit_feature_flags:is_enabled(replaced_delete_queue_transaction));
        v2 ->
            ?assert(
               rabbit_feature_flags:is_enabled(replaced_delete_queue_transaction))
    end,
...
replaced_delete_queue_transaction(Config) ->
    passed = rabbit_ct_broker_helpers:rpc(Config, 0, ?MODULE, replaced_delete_queue_transaction1, [Config]).

replaced_delete_queue_transaction1(_Config) ->
    ...
    %% 1. with the `replaced_delete_queue_transaction' feature flag disabled
    ...
    ?assertEqual(
       ok,
       rabbit_feature_flags:enable(replaced_delete_queue_transaction)),
