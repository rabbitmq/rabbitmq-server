...
init_per_group(_, Config) ->
    ...
    rabbit_sole_conn:ensure_running(),
    Config.

end_per_group(_, Config) ->
    case whereis(rabbit_sole_conn) of
        undefined ->
            ok;
        Pid ->
            gen_server:stop(Pid)
...
refuse_connection_should_refuse_different_user_even_if_existing_dead(_) ->
    ...
    Conn1 = rabbit_sole_conn:conn(Pid1, ?USER1),
...
close_existing_should_close_existing_connection(_) ->
    ...
    ?assertEqual({ok, rabbit_sole_conn:conn(Pid1, ?USER1)},
                 khepri:get(get_store_id(), Path)),
    %% 2 takes the lease from 1
    ?assertEqual(ok, acquire(close_existing, ?VH, ?CID1, ?USER1, Pid2)),
    ?assertEqual({ok, rabbit_sole_conn:conn(Pid2, ?USER1)},
                 khepri:get(get_store_id(), Path)),
...
close_existing_should_refuse_different_user_and_not_kill_existing(_) ->
    ...
    eventually(?_assertNot(is_process_alive(Pid1))),
    ?assertEqual({ok, rabbit_sole_conn:conn(Pid3, ?USER1)},
                 khepri:get(get_store_id(), Path)),
...
try_put(_) ->
    ...
    Pid2 = spawn_disposable(),
    Conn2 = rabbit_sole_conn:conn(Pid2, ?USER1),
    %% new connection manages to replace old connection
    ?assertEqual(ok, rabbit_sole_conn:try_put(Path, Conn1, Conn2)),
    ...
    Pid3 = spawn_disposable(),
    Conn3 = rabbit_sole_conn:conn(Pid3, ?USER1),
    ?assertEqual(error, rabbit_sole_conn:try_put(Path, Conn1, Conn3)),
    ?assertEqual({ok, Conn2}, khepri:get(get_store_id(), Path)),
...
khepri_cas(_) ->
    ...
    Pid2 = spawn_disposable(),
    Pid3 = spawn_disposable(),
    V1 = rabbit_sole_conn:conn(Pid1, ?USER1),
    V2 = rabbit_sole_conn:conn(Pid2, ?USER1),
    V3 = rabbit_sole_conn:conn(Pid3, ?USER1),
...
is_feature_enabled_reflects_flag_state(_) ->
    ?assert(rabbit_sole_conn:is_feature_enabled()),

    ok = meck:expect(rabbit_feature_flags, is_enabled,
                     fun('rabbitmq_4.4.0') -> false end),
    ?assertNot(rabbit_sole_conn:is_feature_enabled()),
    ?assertEqual({error, {feature_flag_disabled, 'rabbitmq_4.4.0'}},
                 rabbit_sole_conn:status()),
    ?assertEqual({error, {feature_flag_disabled, 'rabbitmq_4.4.0'}},
                 rabbit_sole_conn:force_delete(?VH, ?CID1)),
