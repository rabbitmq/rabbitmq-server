...
-define(RA_CLUSTER_NAME, rabbit_sole_conn_ra).
-define(STORE_ID, ?RA_CLUSTER_NAME).
-define(RA_FRIENDLY_NAME, "Sole Conn Enforcement").
-define(RA_SYSTEM, coordination).
-define(TRIGGER_ID, sole_conn_kill_connection).
...
%% {ResourceId, LockRequesterId}, self() keeps concurrent callers
%% mutually exclusive.
-define(BOOTSTRAP_LOCK, {?MODULE, self()}).
...
-rabbit_boot_step({?MODULE,
                   [{description, "sole connection enforcement"},
                    {mfa,         {?MODULE, recover, []}},
                    {requires,    database},
                    {enables,     pre_flight}]}).
...
-record(conn, {pid :: rabbit_types:connection(),
...
evict_node(Node) ->
    ...
    case net_adm:ping(Node) of
        ...
        pang ->
            %% Node is offline, we can't ask it to reset itself.
...
is_resizing() ->
    gen_server:call(?GEN_SERVER_NAME, is_resizing).
    ...
    case members() of
        {ok, Members} ->
            ...
            lists:zipwith3(
                      ...
                      case MetricsResult of
                          #{state := RaftState,
                            ...
                            snapshot_index := SnapIdx} ->
                              %% Failing gracefully to 0 if the machine version
                              %% couldn't be fetched
                              MacVer = case MacVerRes of
                                           {ok, Version} -> Version;
                                           {error, _} -> 0
                                       end,
                              [{<<"Node Name">>, N},
              ...
              end, Nodes, MetricsResults, MacVerResults);
...
-spec force_delete(rabbit_types:vhost(), container_id()) ->
    ok | {error, any()}.
...
is_resizing_remote(Node) ->
    try
        erpc:call(Node, ?MODULE, is_resizing, [], ?ALIVENESS_RPC_TIMEOUT)
    catch
...
recover() ->
    ?LOG_DEBUG("Trying to restart local sole conn Khepri store on ~p", [node()]),
    case ra_directory:uid_of(get_ra_system(), get_store_id()) of
        undefined ->
            ?LOG_DEBUG("Local server name not registered, will start on demand"),
        ...
        _ ->
            ...
            StoreId = get_store_id(),
            ?LOG_DEBUG("Local server name registered, "
                       "restarting local sole conn Khepri store"),
            try
                ...
                Class:Reason ->
                    ?LOG_INFO("~ts: Failed to restart the sole conn Khepri store "
                              "on ~p. Error: ~p:~p",
                              [?MODULE, node(), Class, Reason]),
                    ok
            end
...
-spec acquire(enforcement_policy() | none,
              rabbit_types:vhost(),
              container_id(),
...
refuse_connection_error() ->
    ...
    amqp_error(
      ...
      {{symbol, <<"invalid-field">>}, {symbol, <<"container-id">>}}).
...
close_existing_connection_error() ->
    ...
    amqp_error(?V_1_0_AMQP_ERROR_RESOURCE_LOCKED,
               <<"connection closed because another connection with the same "
                 "container-id was established">>,
               {?SOLE_CONN_ENFORCEMENT, true}).
            ...
            case same_user(ExistingConn, Username) of
                true ->
                    case check_conn(ExistingConn) of
                        true ->
                            log_refuse_connection(VHost, ContainerId, Username,
                                                  ExistingPid, ConnPid),
                        ...
                        _ ->
                            case try_put(Path, ExistingConn, Payload) of
                                ok ->
                                    ok;
                                _ ->
                                    %% Race condition, refusing connection.
                                    %% Technical details are logged in try_put/3.
                                    log_refuse_connection(VHost, ContainerId,
                                                          Username, ExistingPid,
                                                          ConnPid),
                                    {error, refuse_connection}
                            end
                    end;
                false ->
                    log_refuse_connection_no_hijacking(VHost, ContainerId,
                                                       Username, ExistingPid,
                                                       ConnPid),
                    {error, refuse_connection}
            end;
        ...
        {error, ?khepri_error(mismatching_node, #{node_props := #{data := ExistingConn}})} ->
            ...
            case same_user(ExistingConn, Username) of
                true ->
                    case try_put(Path, ExistingConn, Payload) of
                        ok ->
                            log_close_existing(VHost, ContainerId, Username,
                                               ExistingPid, ConnPid),
                            ok;
                        _ ->
                            %% Race condition, refusing connection.
...
%% Iterates through peer nodes and checks if the rabbit_sole_conn process is alive.
find_active_peer([]) ->
    undefined;
find_active_peer([Node | Rest]) ->
    ...
    try erpc:call(Node, erlang, whereis, [?GEN_SERVER_NAME], 1000) of
        Pid when is_pid(Pid) ->
            Node;
        _ ->
...
default_command_options() ->
    maps:merge(?DEFAULT_COMMAND_OPTIONS,
               #{timeout => default_timeout()}).
...
check_conn(#conn{pid = Pid}) ->
    ...
    case Node =:= node() of
        ...
        false ->
            try
                erpc:call(Node, erlang, is_process_alive, [Pid],
                          ?ALIVENESS_RPC_TIMEOUT)
            catch
                error:{erpc, _Reason} ->
    ...
    case whereis(?GEN_SERVER_NAME) of
        undefined ->
            ...
            case OtherMembers of
                [] ->
                    %% No other node to ask and the local store never started:
                    %% the Ra cluster does not exist yet. ra:members/1 has no
                    %% clause for an empty server list and would crash.
                    {error, {no_more_servers_to_try, []}};
                _ ->
...
log_refuse_connection(VHost, CID, Username, ExistingPid, NewPid) ->
    ?LOG_WARNING("Container ID conflict (vhost ~ts, CID ~ts, user ~ts, "
                 "existing connection ~p, new connection ~p). "
                 "Refusing connection.",
                 [VHost, CID, Username, ExistingPid, NewPid]).

log_refuse_connection_no_hijacking(VHost, CID, Username, ExistingPid, NewPid) ->
    ?LOG_WARNING("Container ID conflict (vhost ~ts, CID ~ts, existing connection ~p, "
                 "new connection ~p), user ~ts cannot take ownership. "
                 "Refusing connection.",
                 [VHost, CID, ExistingPid, NewPid, Username]).

log_close_existing(VHost, CID, Username, ExistingPid, NewPid) ->
    ?LOG_WARNING("Container ID conflict (vhost ~ts, CID ~ts, user ~ts, "
                 "existing connection ~p, new connection ~p). "
...
conn_path(VHost, ContainerId)
  when ?IS_KHEPRI_PATH_CONDITION(VHost) andalso
    ...
    ?RABBITMQ_KHEPRI_VHOST_PATH(VHost, [sole_conn, ContainerId]).

kill_connection_sproc_path() ->
    ?RABBITMQ_KHEPRI_ROOT_PATH([sole_conn, kill_connection]).

kill_connection_sproc_trigger_pattern() ->
    ?RABBITMQ_KHEPRI_VHOST_PATH(?KHEPRI_WILDCARD_STAR,
                                [sole_conn,
...
leader_wait_retry_timeout() ->
    application:get_env(rabbit, sole_connection_leader_wait_retry_timeout,
                        ?LEADER_WAIT_RETRY_TIMEOUT).

%% Timeout applied to Khepri operations against the sole connection store.
default_timeout() ->
    application:get_env(rabbit, sole_connection_default_timeout,
