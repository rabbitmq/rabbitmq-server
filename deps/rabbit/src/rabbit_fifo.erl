...
%% the result at ?MAX_MSG_ANNS_SIZE distinct non-reserved keys. Existing
%% keys can still be updated once the cap is reached, but new non-reserved
%% keys beyond the cap are dropped. ?RESERVED_ANN_KEYS are exempt from the
%% cap: see their comments.
...
merge_msg_anns(Existing, New) ->
    %% map_size(Acc) also counts reserved keys, which are exempt from the
    %% cap. Counting them would make the effective cap on non-reserved
    %% keys depend on whether reserved keys happened to be merged in
    %% before or after this call, so a running count of non-reserved keys
    %% is threaded through the fold instead.
    NonReservedCount0 = map_size(Existing) - count_reserved_anns(Existing),
    {Acc, _} =
        maps:fold(
          fun(Key, Value, {AccMap, NonReservedCount}) ->
                  case lists:member(Key, ?RESERVED_ANN_KEYS) of
                      true ->
                          {maps:put(Key, Value, AccMap), NonReservedCount};
                      false ->
                          case maps:is_key(Key, AccMap) of
                              true ->
                                  %% updating an already-present key does
                                  %% not grow the non-reserved count
                                  {maps:put(Key, Value, AccMap),
                                   NonReservedCount};
                              false when NonReservedCount < ?MAX_MSG_ANNS_SIZE ->
                                  {maps:put(Key, Value, AccMap),
                                   NonReservedCount + 1};
                              false ->
                                  {AccMap, NonReservedCount}
                          end
                  end
          end, {Existing, NonReservedCount0}, maps:iterator(New, ordered)),
    Acc.

count_reserved_anns(Anns) ->
    lists:foldl(
      fun(Key, Acc) ->
              case maps:is_key(Key, Anns) of
                  true -> Acc + 1;
                  false -> Acc
              end
      end, 0, ?RESERVED_ANN_KEYS).
