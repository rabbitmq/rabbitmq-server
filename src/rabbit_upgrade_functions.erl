...
-rabbit_upgrade({pending_slave_pids,    mnesia, [policy_version]}).
...
pending_slave_pids() ->
    ok = pending_slave_pids(rabbit_queue),
    ok = pending_slave_pids(rabbit_durable_queue).

pending_slave_pids(Table) ->
    transform(
      Table,
      fun ({amqqueue, Name, Durable, AutoDelete, ExclusiveOwner, Arguments,
            Pid, SlavePids, SyncSlavePids, DSN, Policy, GmPids, Decorators,
      ...
      [name, durable, auto_delete, exclusive_owner, arguments, pid, slave_pids,
       sync_slave_pids, recoverable_slaves, policy, gm_pids, decorators, state,
       policy_version, pending_slave_pids]).
