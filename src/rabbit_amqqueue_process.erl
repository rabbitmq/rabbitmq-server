...
maybe_deliver_or_enqueue(Delivery = #delivery{message = Message},
    ...
    if IsDuplicate -> send_reject_publish(Delivery, Delivered, State1);
       true        -> case {will_overflow(Delivery, State1), Overflow} of
                          {true, 'reject-publish'} ->
...
deliver_or_enqueue(Delivery = #delivery{message = Message,
    ...
    case attempt_delivery(Delivery, Props, Delivered, State1) of
        ...
        {undelivered, State2 = #q{ttl = 0, dlx = undefined,
            ...
            State2#q{backing_queue_state = BQS3, msg_id_to_channel = MTC1};
        {undelivered, State2 = #q{backing_queue_state = BQS2}} ->

            BQS3 = BQ:publish(Message, Props, Delivered, SenderPid, Flow, BQS2),
            {Dropped, State3 = #q{backing_queue_state = BQS4}} =
                maybe_drop_head(State2#q{backing_queue_state = BQS3}),
            QLen = BQ:len(BQS4),
