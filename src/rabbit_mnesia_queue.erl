    ...
    {atomic, Result} =
        mnesia:transaction(
          fun () ->
                  ...
                  {SeqId, transactional_save_state(RS)}
          end),
    rabbit_log:info(" -> ~p", [Result]),
    Result.
...
dropwhile(Pred, S) ->
    ...
    {atomic, {_, Result}} =
        mnesia:transaction(fun () ->
                                   ...
                                   {Atom, transactional_save_state(RS)}
                           end),
    rabbit_log:info(" -> ~p", [Result]),
    Result.
          ...
          fun () ->
                  {DR, RS} =
                      internal_queue_out(
                        ...
                        S),
                  {DR, transactional_save_state(RS)}
          end),
    rabbit_log:info(" -> ~p", [Result]),
    Result.
...
ack(SeqIds, S) ->
    ...
    {atomic, Result} =
        mnesia:transaction(fun () ->
                                   ...
                                   {Guids, transactional_save_state(RS)}
                           end),
    rabbit_log:info(" -> ~p", [Result]),
    Result.
...
tx_rollback(Txn, S) ->
    ...
    {atomic, Result} =
        mnesia:transaction(fun () ->
                                   ...
                                   RS = erase_tx(Txn, S),
                                   {SeqIds, transactional_save_state(RS)}
                           end),
    rabbit_log:info(" -> ~p", [Result]),
    Result.
