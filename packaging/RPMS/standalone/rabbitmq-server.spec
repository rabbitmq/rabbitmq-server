...
Requires: logrotate, openssl = 1.0.0
...
%build
mv rabbitmq_server-%{version}/* .
rm -rf rabbitmq_server-%{version}
...
mkdir -p %{_maindir}
# copy server files (ebin, sbin, etc) to _maindir
cp -r * %{_maindir}/
mkdir -p %{buildroot}%{_rabbit_libdir}
mkdir -p %{buildroot}%{_mandir}

# TODO refactor these functions into one taking 2 args
fix_env () {
    sed -e $1 sbin/rabbitmq-env > sbin/rabbitmq-env.tmp && mv sbin/rabbitmq-env.tmp sbin/rabbitmq-env
}

fix_defaults () {
    sed -e $1 sbin/rabbitmq-defaults > sbin/rabbitmq-defaults.tmp && mv sbin/rabbitmq-defaults.tmp sbin/rabbitmq-defaults
}

fix_env 's:RABBITMQ_HOME="\${SCRIPT_DIR}/..":RABBITMQ_HOME="\${SCRIPT_DIR}/../lib/rabbitmq_server-3.1.3/":'
fix_defaults 's:CLEAN_BOOT_FILE="\${SYS_PREFIX}/releases/3.1.3/start_clean":CLEAN_BOOT_FILE="\${RABBITMQ_HOME}/releases/3.1.3/start_clean":'
fix_defaults 's:SASL_BOOT_FILE="\${SYS_PREFIX}/releases/3.1.3/start_sasl":SASL_BOOT_FILE="\${RABBITMQ_HOME}/releases/3.1.3/start_sasl":'
fix_defaults 's:SYS_PREFIX=\${RABBITMQ_HOME}:SYS_PREFIX=:'

mv sbin %{buildroot}%{_rabbit_libdir}/bin
